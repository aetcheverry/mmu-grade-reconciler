...
```bash
conda activate mmu-grade-reconciler
```
...
Use the **Reset** button to clear all files and start a new comparison.
...
- When running the exe for the first time, Windows may show a SmartScreen warning. Click **More info** then **Run anyway** to proceed. This happens because the exe is not digitally signed.
